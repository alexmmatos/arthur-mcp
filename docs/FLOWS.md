...
- Arthur-managed authorization-server metadata is published at `/.well-known/oauth-authorization-server/oauth/server/:serverId` only. The previous unscoped `/.well-known/oauth-authorization-server` (no `serverId`) was removed: it unconditionally advertised OAuth support (with unresolved `{serverId}` placeholders) for every server on the instance, including ones with OAuth disabled, which made AI clients that probe that root path as a discovery fallback wrongly believe OAuth was required.
