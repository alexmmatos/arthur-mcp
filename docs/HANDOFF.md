...
- Fixed Claude/ChatGPT reporting that OAuth was required to connect to an MCP server that had OAuth disabled (user-reported: "provavelmente ele está informando que existe oauth mesmo sem existir"):
  - Root cause: `OAuthController.discovery()` (`api/src/oauth/oauth.controller.ts`) served `GET /.well-known/oauth-authorization-server` (no `serverId`) unconditionally for any request, returning a valid-looking OAuth Authorization Server Metadata document for every MCP server on the instance — including ones with `oauthConfig.mode === 'none'` — and with `authorization_endpoint`/`token_endpoint` containing a literal, never-substituted `{serverId}` placeholder that could never resolve to a real flow. AI clients that fall back to probing this root well-known path (a common RFC 8414 discovery pattern) concluded OAuth was mandatory and tried to follow the broken URLs, blocking the "add MCP server" flow even for servers configured with just an Access Key or no auth at all.
  - This unscoped endpoint was architecturally incompatible with the product's per-server, multi-tenant OAuth model (each server has its own independent `oauthConfig`), was not referenced by any frontend code, and had no test coverage — confirmed by grepping `src/` and `api/src/**/*.spec.ts`.
  - Fix: removed the `discovery()` handler entirely. The already-correct, per-server-scoped endpoints remain the only discovery surfaces: `/.well-known/oauth-protected-resource/api/mcp/server/:serverId` (404s when that server's mode is `none`) and `/.well-known/oauth-authorization-server/oauth/server/:serverId` (404s unless that server's mode is `managed`). Also removed the now-dead exact-path exclusion entry for the bare `.well-known/oauth-authorization-server` route from the global `/api` prefix config in both `api/src/main.ts` and `api/src/vercel.ts` (the existing `.well-known/oauth-authorization-server/*` wildcard entry already covers the scoped route).
  - Documentation synchronized in `docs/FLOWS.md` ("MCP OAuth Client" section) and `docs/ROADMAP.md`.
  - Validation: `npm run build --prefix api` clean; `npm test --prefix api -- oauth mcp-api-key` — 5 suites / 25 tests passing (no test referenced the removed endpoint, confirming it was dead code).
