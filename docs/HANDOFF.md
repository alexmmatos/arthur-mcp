...
- Added `docs/MCP_APPS_PLAN.md` as the implementation contract for the new Apps domain on `feat/mcp-apps`. It defines the source -> Tool -> declarative View model, `/apps` routes, `mcp-apps` API, `mcp_apps` persistence shape, `apps_view/create/edit/delete` permissions, official `ui://`/MIME/structured-result behavior, six delivery phases, acceptance criteria, security constraints, risks, and explicit non-goals. Phase 0 is complete and the initial backend work is tracked as in progress rather than delivered.
