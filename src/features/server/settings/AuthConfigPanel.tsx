...
export function AuthConfigPanel({ projectId, initialAuth, onChange }: AuthConfigPanelProps) {
  ...
  const [token, setToken] = useState((initialAuth as { token?: string })?.token ?? '')
  const [keyName, setKeyName] = useState((initialAuth as { name?: string })?.name ?? '')
  const [keyValue, setKeyValue] = useState((initialAuth as { value?: string })?.value ?? '')
  const [keyIn, setKeyIn] = useState<'header' | 'query'>((initialAuth as { in?: 'header' | 'query' })?.in ?? 'header')
  const [basicUser, setBasicUser] = useState((initialAuth as { username?: string })?.username ?? '')
  const [basicPass, setBasicPass] = useState((initialAuth as { password?: string })?.password ?? '')
  const [oauthTokenUrl, setOauthTokenUrl] = useState((initialAuth as { tokenUrl?: string })?.tokenUrl ?? '')
  const [oauthClientId, setOauthClientId] = useState((initialAuth as { clientId?: string })?.clientId ?? '')
  const [oauthClientSecret, setOauthClientSecret] = useState((initialAuth as { clientSecret?: string })?.clientSecret ?? '')
  const [oauthScope, setOauthScope] = useState((initialAuth as { scope?: string })?.scope ?? '')
  const [customHeaders, setCustomHeaders] = useState<{ name: string; value: string }[]>(
    (initialAuth as { headers?: { name: string; value: string }[] })?.headers ?? [{ name: '', value: '' }],
  )
          ...
          <HelpButton title="API Authentication">
            ...
            <Box component="ul" sx={{ mt: 0, mb: 1, pl: 2.5 }}>
              {([
                ...
                ['API Key', 'Adds the key as a custom header, for example X-API-Key, or as a query parameter. Check your API documentation for the exact field name.'],
                ...
                ['Custom Headers', 'Add any arbitrary HTTP headers. Useful for APIs with non-standard authentication schemes or when you need to pass multiple headers, such as X-Tenant-Id plus X-Auth-Token.'],
            ...
            <Typography variant="body2" gutterBottom>
              <strong>Important security note:</strong> credentials are stored in the database. Use tokens and keys with the <em>minimum required scope</em>; if a credential is exposed, a limited-scope key reduces the blast radius.
            </Typography>
            <Typography variant="body2">
              These credentials are separate from the <strong>MCP Authentication key</strong>, which protects the MCP endpoint. One controls who can call Arthur; the other controls how Arthur calls your API.
            </Typography>
          </HelpButton>
        </Box>
        ...
        <Box display="flex" flexDirection="column" gap={1.5}>
          ...
          <FormControl size="small" fullWidth>
            ...
            <Select
              ...
              label="Send as"
              onChange={(e) => {
                const v = e.target.value as 'header' | 'query'
                setKeyIn(v)
                scheduleSave({ type: 'api-key', name: keyName, value: keyValue, in: v }, 0)
              }}
            >
      ...
      {authType === 'oauth2-client' && (
        <Box display="flex" flexDirection="column" gap={1.5}>
          <TextField
            size="small"
            fullWidth
            label="Token URL (token endpoint)"
      ...
      {authType === 'custom' && (
        <Box display="flex" flexDirection="column" gap={1}>
          <Typography variant="caption" color="text.secondary" mb={0.5}>
            Add any HTTP header to the request, for example <code>X-Tenant-Id</code> or <code>X-Auth-Token</code>.
          </Typography>
