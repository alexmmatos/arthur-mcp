  ...
  const scheduleSave = useCallback((payload: { enabled: boolean; requestsPerMinute: number }, delay = 700) => {
    ...
    timerRef.current = setTimeout(async () => {
      ...
      if (p.requestsPerMinute < 1 || p.requestsPerMinute > 10000) {
        setSaveError('Value must be between 1 and 10,000.')
        setSaveStatus('error')
        return
      }
          ...
          <Box display="flex" alignItems="center" gap={0.5}>
            ...
            <HelpButton title="Request Limit">
              <Typography variant="body2" gutterBottom>
                Caps the number of MCP requests this server accepts per minute. When the limit is exceeded, the server responds with <strong>HTTP 429 (Too Many Requests)</strong> and a <code>Retry-After</code> header, so the AI client should wait before retrying.
              </Typography>
              <Typography variant="body2" gutterBottom>
              ...
              <Box component="ul" sx={{ mt: 0, mb: 1, pl: 2.5 }}>
                ...
                <Box component="li"><Typography variant="body2"><strong>Fair usage:</strong> if multiple AI clients share this endpoint, a limit ensures no single client monopolizes the quota.</Typography></Box>
              </Box>
              <Typography variant="body2" gutterBottom>
                <strong>How to set the right limit:</strong> check your upstream API's documented rate limit, for example 100 req/min, and set Arthur's limit slightly below it to leave headroom.
              </Typography>
